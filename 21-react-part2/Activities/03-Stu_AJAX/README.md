...
* Add another method to the `API` module that, instead of searching for a specific title, does a more generic search using the `?s=` endpoint. This means it will return an _array_ of movies, though, so the `OmdbContainer.js` file will need to be restructured to map an array of `MovieDetail` components.
