...
* In order to return separate JSX elements from a function, all of the higher level elements need a single parent, usually a `div`.

* All JSX tags must either have an adjacent tag or else have a self-closing forward slash (e.g. `<br/>`).

* `className` must be used to describe an element's `class` property since `class` is a reserved word in JavaScript.

* Don't copy/paste `style` attributes. They are used differently in JSX. Trying to use something like `style="width: 18rem"` will throw an error.
